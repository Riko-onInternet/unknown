...
export default function SeriesModal({
  ...
  const serie = seriesDatabase.find((s) => s.id === seriesId);

  useEffect(() => {
    if (serie && serie.episodes.length > 0) {
      setSelectedSeason(prev => ({
        ...prev,
        [serie.id]: serie.episodes[0].season
      }));
    }
  }, [seriesId, serie]);
      ...
      <ModalContent>
        <ModalBody className="modal-body-series bg-tertiary">
          <div className="relative h-max">
            ...
            <div className="absolute bottom-0 left-0 w-full linear-bottom">
              <div className="flex flex-col gap-3 mb-6">
                ...
                <div className="input-series w-max">
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      toggleSavedSeries(serie.id);
                    }}
                    className={`${savedSeries.includes(serie.id) ? "list-active" : ""}`}
                  >
                      ...
                      >
                        {Array.isArray(tab.content) &&
                          tab.content.map((content) => (
                            <div className="flex flex-col" key={content.title}>
                              ...
                              {content.title === "Stagioni" && (
                                <p>
                                  {serie.seasons}{" "}
                                  {serie.seasons > 1 ? "Stagioni" : "Stagione"}
                                </p>
                              )}
                              {content.title === "Anno" && (
                                <p>
                              ...
                              {content.title === "Generi" && (
                                <div className="flex flex-col">
                                  {serie.generi.slice(0, 3).map((generi) => (
                                    <div
                                      key={generi.id}
                                      className="flex flex-col"
                                    >
                        ...
                        {serie.subtitle && serie.subtitle.length > 0 && (
                          <div className="flex flex-col">
                            ...
                            {serie.subtitle.map((subtitle) => (
                              <div key={subtitle.id} className="flex flex-col">
                                {subtitle.name}
                              </div>
                            ))}
                          </div>
